...
function displayQuestion(index) {
    if(index<questions.length){
        ...
        let questionBox = document.getElementById("question");
        questionBox.innerText="done"
         let op= document.querySelectorAll(".answer");
         op.forEach((ops)=>{
            ops.remove()
            btn.disabled=true
         })
...
function checkAnswer() {
    ...
    submitButton.addEventListener("click", (e) => {
        ...
        if (answer === correctAnswer && index<questions.length) {
            index = index + 1;
            ...
            let questionBox = document.getElementById("question");
            questionBox.innerText = `Sorry, its worng answer\n Your score is ${score}`
